...
function openEnvelope() {
    ...
    if (!wrapper.classList.contains('open')) {
        ...
        // รอให้กระดาษเริ่มโผล่ก่อนค่อยพ่นหัวใจ (0.8 วินาที)
        setTimeout(() => {
            for (let i = 0; i < 25; i++) {
...
function increaseYes() {
    ...
    const scale = 1 + (parseInt(yes.getAttribute('data-count') || 0) + 1) * 1.5;
    yes.setAttribute('data-count', (parseInt(yes.getAttribute('data-count') || 0) + 1));
    yes.style.transform = `scale(${scale})`;
    if(scale > 6) { no.style.display = 'none'; yes.style.width = '100%'; yes.style.height = '100%'; yes.style.position = 'fixed'; yes.style.top = '0'; yes.style.left = '0'; yes.style.zIndex = '999'; }
}

function checkPass() {
    if(document.getElementById('pass-input').value === "1402") { nextPage(4); }
    else { document.getElementById('pass-hint').innerText = "ผิดจ้า! ลองใหม่อีกที"; }
}
...
const msgs = [{t: "😊", s: "right"}, {t: "ดีครับบ", s: "left"}, {t: "ดีค้าบ", s: "right"}, {t: "ชื่ออาราย", s: "right"}, {t: "ปามมี่ค้าบบ", s: "left"}, {t: "หวัดดีแบงค์", s: "left"}, {t: "เรากันน", s: "right"}, {t: "5555555555555555", s: "right"}, {t: "พลาด", s: "left"}, {t: "เอ้าดูชื่อดิ", s: "left"}, {t: "ขอโทษค้าบบบ", s: "left"}, {t: "ละพิมพ์แบบมั่นใจด้วย", s: "right"}, {t: "น่ารักงับบ", s: "right"}];
function startChat() {
...
function initPuzzle() {
    ...
    for(let i=0; i<9; i++) {
        ...
        p.onmousedown = p.ontouchstart = (e) => {
            e.preventDefault(); let mv = e.type==='mousedown'?'mousemove':'touchmove'; let up = e.type==='mousedown'?'mouseup':'touchend';
            const onMove = (me) => { let cx = me.clientX || me.touches[0].clientX; let cy = me.clientY || me.touches[0].clientY; let r = board.getBoundingClientRect(); p.style.left = (cx-r.left-50)+"px"; p.style.top = (cy-r.top-50)+"px"; };
            document.addEventListener(mv, onMove); document.onmouseup = document.ontouchend = () => {
                document.removeEventListener(mv, onMove); const tx = (i%3)*100; const ty = Math.floor(i/3)*100;
                if(Math.abs(parseInt(p.style.left)-tx)<30 && Math.abs(parseInt(p.style.top)-ty)<30) { p.style.left = tx+'px'; p.style.top = ty+'px'; p.style.pointerEvents="none"; solved++; if(solved===9) document.getElementById('puzzle-next').style.display='block'; }
            };
        };
        board.appendChild(p);
    }
...
function initScratch() {
    ...
    const scratch = (e) => { const r = canvas.getBoundingClientRect(); const cx = (e.clientX || e.touches[0].clientX) - r.left; const cy = (e.clientY || e.touches[0].clientY) - r.top; ctx.beginPath(); ctx.arc(cx, cy, 20, 0, 6.28); ctx.fill(); };
